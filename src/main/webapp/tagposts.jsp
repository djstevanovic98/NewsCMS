...
<div class="container">
    <form method="POST" id="newPost-form">
        <h1 class="naslov">RAF Post by tag</h1>
        <div id="post-links">
...
<script>
    if(sessionStorage.getItem('Tpage') == null){
        sessionStorage.setItem('Tpage',0);
    }
    let tag = sessionStorage.getItem('sacuvanTag');

    fetch('/api/posts/' + tag + '/' + sessionStorage.getItem('Tpage') + '/tagPostovi', {
        method: 'GET'
    }).then(response => {
    ...
    function povecajStranu(){
        var strana = parseInt(sessionStorage.getItem('Tpage')) + 1;
        sessionStorage.setItem('Tpage', strana);
        location.reload();
    }
